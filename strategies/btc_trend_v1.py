...
# ---------------------------------------------------------------------------
# Minimal local data contracts.
# Replace/adapt these to match your actual core/models.py Candle and Signal
# dataclasses if they differ in shape. Kept intentionally simple and
# explicit so the mapping is obvious.
# ---------------------------------------------------------------------------

@dataclass(frozen=True)
class Candle:
    timestamp: object  # keep as whatever your real Candle uses (datetime, int, etc.)
    open: float
    high: float
    low: float
    close: float
...
class Signal:
    symbol: str
    direction: str  # "BUY" only in v1
    entry_price: float
    initial_stop: float
    strategy: str = "btc_trend_v1"
